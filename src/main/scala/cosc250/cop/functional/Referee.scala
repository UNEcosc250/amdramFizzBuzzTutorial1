...
/**
  ...
  */
case class RefereeState(from:Int, to:Int, players:List[Recipient[PlayerHears]]) {
  ...
  def eliminateAndNext(p:Recipient[PlayerHears]):RefereeState = {
...
/**
  ...
  */
def referee(state:RefereeState): MessageHandler[RefereeHears] = MessageHandler { (msg, context) =>
